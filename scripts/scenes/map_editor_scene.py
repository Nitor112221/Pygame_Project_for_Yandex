...
class EditorScene:
    def __init__(self, screen, virtual_surface: pygame.Surface, switch_scene, settings):
        ...
        self.new_surface = pygame.Surface((screen.get_width(), screen.get_height()))
    ...
    def transport_position_group_sprite(self, value='+', measurement='x'):
        for sprite in self.group_tile_on_board:
            if measurement == 'x':
                if value == '-':
                    sprite.rect = sprite.rect.move(-10, 0)
                elif value == '+':
                    sprite.rect = sprite.rect.move(10, 0)
            if measurement == 'y':
                if value == '-':
                    sprite.rect = sprite.rect.move(0, -10)
                elif value == '+':
                    sprite.rect = sprite.rect.move(0, 10)

    def increase_tile(self, value='+'):
        for sprite in self.group_tile_on_board:
            if value == '+':
                cur_image = sprite.image
                cur_size = sprite.rect.width
                new_image_sprite = pygame.transform.scale(cur_image, (cur_size + 10, cur_size + 10))
                sprite.image = new_image_sprite
            elif value == '-':
                cur_image = sprite.image
                cur_size = sprite.rect.width
                new_image_sprite = pygame.transform.scale(cur_image, (cur_size - 10, cur_size - 10))
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    x_size = self.new_surface.get_size()[0]
                    y_size = self.new_surface.get_size()[1]
                    scaled_surface = pygame.transform.scale(self.new_surface, (x_size + 1, y_size + 1))
...
class Board:
    ...
    def chek_clicked_on_board(self, coor, current_tile, group_tile_on_board, id_):
        for i in range(len(self.coordinate_cell)):
            for j in range(len(self.coordinate_cell[i])):
                if self.coordinate_cell[i][j][0] < coor[0] < \
                        (self.coordinate_cell[i][j][0] + self.cell_size) and \
    ...
    def increase_tile(self, value='+'):
        for sprite in self.group_tile_on_board:
            if value == '+':
                cur_image = sprite.image
                cur_size = sprite.rect.width
                sprite.rect.width = sprite.rect.width + 1
                new_image_sprite = pygame.transform.scale(cur_image, (cur_size + 1, cur_size + 1))
                sprite.image = new_image_sprite
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    board.chek_clicked_on_board(event.pos, self.current_tile, self.group_tile_on_board, self.id_)
